...
export class SolanaService {
  ...
  async transfer(from: string, to: string) {
    const source = await createAssociatedTokenAccountIdempotent(this.connection, this.keypair, new PublicKey(from), this.keypair.publicKey, { commitment: "confirmed" }, TOKEN_2022_PROGRAM_ID);
    this.logger.log(`Source account: ${source}`);
    const destination = await createAssociatedTokenAccountIdempotent(this.connection, this.keypair, new PublicKey(from), new PublicKey(to), { commitment: "confirmed" }, TOKEN_2022_PROGRAM_ID);
  ...
  private async createTokenAndMint(ipfsHash: string): Promise<string> {
    try {
      ...
      const metadata: TokenMetadata = {
        ...
        symbol: 'ESG',
        uri: "",
        additionalMetadata: [["tag", 'Link to IPFS'], ["data", `${PINATA_ENDPOINT}/ipfs/${ipfsHash}?pinataGatewayToken=${PINATA_JWT}`]],
      };
  ...
  async generateESGReport(data: any[], companyData: any): Promise<Buffer> {
    try {
      ...
      doc.font('Times-Bold')
        .fontSize(16)
        .text(`${companyData?.companyShortName} Corporate ESG Report: ESRS E1 - CLIMATE CHANGE Disclosure`, { align: 'center' })
        .moveDown(1.5)
        .fontSize(14)
      ...
      table.rows.forEach((row) => {
        ...
        Object.values(row).forEach((cell, columnIndex) => {
          ...
          doc
            .font('Times-Roman')
            .fontSize(11)
            .text(text, columnPositions[columnIndex], y, { width: columnWidths[columnIndex], height, align: "left" })
        });
        y += (height + 20)
      });
  ...
  async uploadToPinata(pdfBuffer: Buffer): Promise<string> {
    try {
      ...
      this.logger.error(`Error uploading file to Pinata. Message: `, error?.message, 'Stack Trace: ', error?.stack);
      throw error;
    }
  }
  ...
  async transferToSolana(ipfsHash: string, walletAddress: string): Promise<string> {
    try {
      ...
      const transaction = this.createTokenAndMint(ipfsHash).then(async (publicKey) => await this.transfer(publicKey, walletAddress));
      return transaction;
    } catch (error) {
